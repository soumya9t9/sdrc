...
export class TableComponent implements OnInit, OnChanges {
  ...
  @Output() onShowRemarkButtonClicked: EventEmitter<any> = new EventEmitter<any>();
  ...
  setDuplicateFixedTable() {
    ...
    // if($("#"+this.id).parent()[0].scrollWidth == $("#"+this.id).parent()[0].clientWidth){
      if($(window).width() <= 768){
        ...
        $("#header-fixed" + this.id).width(this.fixTableConfig.width - 12)
      }
    // }
  ...
  checkChangesInValue():boolean {
    let mappedRowData = this.arrayToObject(this.rowData)
    let mappedRowDataBackup = this.arrayToObject(this.rowDataBackup)
    let flag = false
    for (let i = 0; i < Object.keys(mappedRowData).length; i++) {
      const rowKey = Object.keys(mappedRowData)[i];
      if(mappedRowDataBackup[rowKey] != mappedRowData[rowKey]){
        flag = true;
      }
    }
    return flag
  ...
  checkRejectedNullValues(): boolean {
    let flag = false;
    for (let i = 0; i < this.rowData.length; i++) {
      if(this.rowData[i].Status == 'red' && (this.rowData[i]['Data Value'] == null || this.rowData[i]['Data Value'] == '')) {
        flag = true;
      }
  ...
  public actionClicked(btnClass, rowObj): void {
    let emittedData: any = {indicators: this.rowData, rowObj: rowObj};
    this.onActionButtonClicked.emit(emittedData);
  }

  public submitData() {
    let emittedData:any = {indicators: this.rowData}
    this.onActionButtonClicked.emit(emittedData);
  }
  ...
  public showRemarkButtonClicked(remark) {
    let emittedData: any = {remark: remark};
    this.onShowRemarkButtonClicked.emit(emittedData);
  }

   arrayToObject(array) {
  return array.reduce((obj, item) => {
    obj[item['Indicator Id']] = item['Data Value']
    return obj
  }, {})    }
}
